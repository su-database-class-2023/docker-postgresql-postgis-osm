...
def query_geopandas(db):
    ...
    # 埼玉県内市町村ごとの2019年1月の休日昼間人口と休日夜間人口の差
    sql = "WITH \
            day AS \
            ...
            night AS \
                (SELECT p.name, d.prefcode, d.year, d.month, d.population, p.geom \
                    ...
                    WHERE d.dayflag='0' AND \
                          ...
                          d.year='2019' AND \
                          d.month='01') \
            SELECT poly.name_2, sum(day.population)-sum(night.population) AS dif, poly.geom \
                FROM day, night, adm2 AS poly \
                WHERE poly.name_1='Saitama' AND \
                    st_within(day.geom,poly.geom) AND \
                    st_within(night.geom,poly.geom) \
...
def main():
    ...
    plt.savefig('/work/python/sample_mapping_12.jpg') #specify filename
